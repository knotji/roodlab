...
export async function writeSnapshot(lotteryId:string,draws:LotteryDraw[],freshness?:SnapshotFreshness,metadata?:{currentSourceResultDate?:string|null;providerResultStatus?:"normal"|"suspended"|"unknown";providerStatusRaw?:string;syncOutcome?:SyncOutcome}):Promise<Snapshot>{const now=new Date().toISOString(),snapshot:Snapshot={lotteryId,syncedAt:now,lastSuccessfulSyncAt:now,source:"AllHuay",draws,historyVersion:computeHistoryVersion(lotteryId,draws),drawCount:draws.length,latestCompleteDrawDate:draws.find(isCompleteDraw)?.drawDate??null,currentSourceResultDate:metadata?.currentSourceResultDate??draws[0]?.drawDate??null,providerResultStatus:metadata?.providerResultStatus??"unknown",...(metadata?.providerStatusRaw?{providerStatusRaw:metadata.providerStatusRaw}:{}),syncOutcome:metadata?.syncOutcome??"updated",...(freshness?{freshness}:{})};safeId(lotteryId);if(hasDatabase())await writePostgresSnapshot(snapshot);else await atomicWriteFile(path.join(historyDir,`${lotteryId}.json`),JSON.stringify(snapshot,null,2));snapshotMemory.set(lotteryId,timed(snapshot));return snapshot}
