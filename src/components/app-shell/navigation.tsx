...
export function AppSidebar<T extends string>({ items, active, onNavigate, statusTitle, statusDetail }: { items: readonly NavigationItem<T>[]; active: T; onNavigate: (id: T) => void; statusTitle: string; statusDetail: string }) {
  return <aside className="sidebar" aria-label="เมนูหลัก"><div className="brand"><span aria-hidden="true">R</span><div><strong>RoodLab</strong><small>Statistical explorer</small></div></div><nav>{items.map((item) => <Button variant="plain" size="auto" key={item.id} className={cn(active === item.id && "active")} onClick={() => onNavigate(item.id)} aria-current={active === item.id ? "page" : undefined}><item.icon />{item.label}</Button>)}</nav><div className="sidebar-note"><Database /><div><strong>{statusTitle}</strong><small>{statusDetail}</small></div></div></aside>;
}
